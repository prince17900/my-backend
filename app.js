...
app.use("/compressed", express.static(compressedDir)); // Serve compressed files

app.post("/compress", upload.single("pdf"), (req, res) => {
  const inputPath = req.file.path;
  const outputPath = path.join(compressedDir, `compressed-${req.file.filename}`);
  ...
  fs.copyFile(inputPath, outputPath, (err) => {
    ...
    res.json({
      success: true,
      downloadUrl: `${req.protocol}://${req.get("host")}/compressed/compressed-${req.file.filename}`
    });
  });
});
